...
int main(int argc,char **argv)
{
  ...
  odom.twist.twist.linear.x = atof(argv[6]);
  ...
  *tmpcp=0; // remove extension from png file name
  ...
  for(i=0;i<NUMSAMP;i++)
    {
      samples[i].x += ((gsl_rng_uniform_pos(srng1)-0.5)*map->res);
      samples[i].y += ((gsl_rng_uniform_pos(srng2)-0.5)*map->res);
      samples[i].theta += (M_PI * 2.0* (gsl_rng_uniform_pos(srng3)-0.5) /
			   map->nAngles);
    }
  ...
  for(i=0;i<NUMSAMP;i++)
    {
      ...
      //if(s_initial.angle == 0)
	finalmapdata[s_initial.row][s_initial.col]+=1.0;
    }
